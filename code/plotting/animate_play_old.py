...
data_path = (script_dir / "../../data/train").resolve()


# Modify the variables below to plot your desired play
tracking_file = "input_2023_w01.csv"
game_id = 2023090700
play_id = 101

tracking_file = data_path / tracking_file
...
absolute_yd_line = df_tracking.absolute_yardline_number.values[0]
play_going_right = (
...
first_down_marker = (
    (line_of_scrimmage + df_tracking.yardsToGo.values[0])
    if play_going_right
    else (line_of_scrimmage - df_tracking.yardsToGo.values[0])
...
print(
    f"Line of Scrimmage: {line_of_scrimmage}, First Down Marker: {first_down_marker}"
)
...
# Frame Info
sorted_frame_list = df_tracking.frameId.unique()
...
for frameId in sorted_frame_list:
    ...
    # Plot Players
    for club in df_tracking.club.unique():
        plot_df = df_tracking[
            (df_tracking.club == club) & (df_tracking.frameId == frameId)
        ].copy()
        if club != "football":
            hover_text_array = []
            ...
            if (
                ...
                ballcarrier_df = df_tracking[
                    (df_tracking.nflId == ballcarrierId)
                    & (df_tracking.frameId == frameId)
                ].copy()
                data.append(
                    go.Scatter(
                        x=ballcarrier_df["x"],
