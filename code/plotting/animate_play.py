...
from animation_assets import colors, drop_list
...
def main():
    ...
    df_output_merged_premap = df_output_merged.drop(drop_list, axis=1)

    # Create a mapping of player roles / names from pre-pass data
    role_mapping = df_input_final[['game_id', 'play_id', 'nfl_id', 'player_role']].drop_duplicates()
    df_output_final = df_output_merged_premap.merge(
        role_mapping,
    ...
    layout = go.Layout(
        ...
        title=f"GameId: {args.game_id}, PlayId: {args.play_id}<br>{game_clock} {quarter}Q, Pass at Frame {pass_frame_id}"
        + "<br>" * 19
        + f"{play_description}",
